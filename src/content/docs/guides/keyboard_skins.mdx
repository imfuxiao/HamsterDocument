...
| 函数名 | 入参 | 返回值 | 说明 |
| --- | --- | --- | --- |
| `$getRimeOptionState` |  字符串 | 布尔值 | 获取 rime 引擎 option 的状态。<br />如 `ascii_state` |
...
:::note[浮动键盘]
浮动键盘最大宽度、高度为当前键盘区域的 85%。当需要缩小浮动键盘的区域时，可使用 `insets` 属性设置。

`insets` 属性可用于 `keyboardType` 的样式中，也可用于 `keyboardType` 中的 `backgroundStyle` 中。
:::
