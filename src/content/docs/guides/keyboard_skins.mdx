...
注意：YAML 锚点语法需要递归的方式获取对应的值，考虑对键盘性能的影响，「仓」内置的「键盘皮肤」功能中，在点击选中皮肤后，会将配置文件中的锚点语法转为普通的文本值（此时文件中的注释信息会丢失）。如果您不使用「仓」的界面配置，需要手工转换锚点, 并提供转换后的文件，注意转换后的文件为 `.文件名.keyboard`。

## 功能说明

「键盘皮肤」是「仓输入法」（以下简称「仓」）全新的自定义键盘功能，您可以使用此功能定制个性化的键盘。

「键盘皮肤」与已有功能[「自定义键盘」](https://ihsiao.com/apps/hamster/docs/CustomKeyboard.html)都可以完成对键盘的自定义，但「键盘皮肤」的功能更为强大，比如，支持跨行的布局，支持图片作为背景等等。
...
| name | 字符串，必须 | 皮肤名称 |
| author | 字符串，必须 | 作者 |
| fontFace | 对象，非必须 | 皮肤应用的字体，具体请查阅字体设置 |
| pinyin | 对象，必须 | 中文键盘样式，具体请查阅键盘设置 |
| alphabetic | 对象，必须 | 英文键盘样式，具体请查阅键盘设置 |
| numeric | 对象，必须 | 数字键盘样式，具体请查阅键盘设置 |
| symbolic | 对象，必须 | 符号键盘配置，具体请查阅键盘设置 |

### 字体设置

在 `config.yaml` 文件中通过 `fontFace` 属性声明字体。具体[查阅 fontFace 参数说明](https://www.notion.so/1320147ebd8a80da8f17f4d02e2894c1?pvs=21)。

重要：字重属性 fontWeight 会影响字体的设置。

> 注意两种情况：
1. 当在「皮肤」中设置了字体后，「仓」中设置的字体会无效；
2. 当「仓」设置了字体功能，而「皮肤」没有使用字体，则「皮肤」会应用「仓」中设置的字体。
>
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| iPhone | 对象 | 表示当前设备为 iPhone 时采用的键盘配置，具体请查阅 iPhone 参数 |
| iPad | 对象 | 表示当前设备为 iPad 时采用的键盘配置。具体请查阅 iPad 参数 |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| `insets` | 对象 | 指文字距离预编辑区四周的距离，用于微调预编辑区显示文字的位置，具体内容可查阅 insets 属性 |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| normalImage | 对象 | 可用来设计预编辑区的背景图片。具体查阅 normalImage 属性 |
...
| --- | --- | --- |
| `textColor` | 字符串 | 预编辑区文字颜色 |
| `fontSize` | 浮点 | 预编辑区文字大小，具体查阅 fontSize 属性 |
| `fontWeight` | 枚举 | 预编辑区字重，具体查阅 fontWeight 文档属性 |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| normalImage | 对象 | 背景样式图片，具体请查阅 normalImage 属性 |
...
| action | 字符串或对象 | 按键的动作，具体请查阅行为 Action |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| normalImage | 对象 | 一般状态下，按键的前景图片，具体请查阅 normalImage 属性 |
| highlightImage | 对象 | 按下状态时，按键的前景图片，具体请查阅 highlightImage 属性 |

当使用文字时，可以使用下面属性：

| 属性 | 类型 | 说明 |
| --- | --- | --- |
| text | 字符串 | 定义按键显示的文字，可使用 JavaScript 动态显示当前文字。具体请查阅 JavaScript 扩展 |
| normalColor | 字符串 | 一般状态下文字颜色，可参考颜色格式 |
| highlightColor | 字符串 | 按下状态时，文字颜色，可参考颜色格式 |
| fontSize | 浮点 | 字体大小，可查阅 fontSize 属性 |
| fontWeight | 枚举 | 字重，可查阅 fontWeight 属性 |

#### secondaryButtonStyle

除主键外的其他按键的样式。数组类型，最多添加 7 个样式引用。每个样式的设置与 primaryButtonStyle 设置方式相同，具体请查阅 primaryButtonStyle 样式。
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| normalImage | 对象 | 一般状态下，按键的前景图片。具体请查阅 normalImage 参数。 |
| highlightImage | 大中型 | 按下状态时，按键的前景图片。具体请查阅 highlightImage 参数。 |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| insets | 对象类型 | 用于调整垂直划动面板的内距。具体请查阅 insets 参数。 |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| normalImage | 对象 | 背景图片设置，具体请查阅 normalImage 属性。 |

- candidateStyle 候选字样式设置，它包含以下属性

| 属性 | 类型 | 说明 |
| --- | --- | --- |
| insets | 对象类型 | 候选字区域内距调整，具体请查阅 insets 参数。 |
...
| --- | --- | --- |
| name | 字符串 | 操作名称 |
| action | 字符串或对象 | 行为，具体请查阅行为 Action |
...
上例中 `row1Style` 是 HStack 自定义样式的值，它包含一个 `size` 属性，通过 size 可以设置分割区域的宽度或高度。具体请查阅 size 属性。
...
`bounds` 与 `size` 基本相同，都是调整大小的，只是 `bounds` 多了一个 `alignment` 属性，指示当显示宽度或高度与物理尺寸大小不一致时，需要向那个方向对齐。如上图所示中的 a 键需要向右对齐，而 l 键需要向左对齐。具体参数请查阅 bounds 属性。
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| normalImage | 对象 | 必须，普通状态下的背景图片。具体参数请查阅 normalImage。 |
| highlightImage | 对象 | 可选，当按键按下时的背景图片，具体参数请查阅 highlightImage。 |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| type | 枚举 | 必须，目前固定值为 `original`，样式类型，用来表示当前背景使用原生属性设置。 |
| insets | 对象 | 非必须，用来设置背景的内距，当内距为0时，背景尺寸等于Cell的显示尺寸，此时Cell之间没有间距。 |
...
| shadowOffset | 对象 | 非必须，阴影偏移量，具体请查阅 offset 。 |
...
| center | 对象 | 必须。调整字体显示位置。具体请查阅 center 属性。 |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| insets | 对象 | 调整按键气泡的内距。 <br/> 当值为空，或值全部为零时，使用默认值： `insets: { top: 6, bottom: 6, left: 8, right: 8 }` |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| action | 字符串或对象 | 点击后的行为。具体请查阅行为 Action。 |
...
注意: APNG 文件大小请控制在 30 KB 以内（越小越好），动画时长控制在 1s 以内。过大的图片会导致键盘内存超限，被系统杀进程。
...
如果您的九键方案不需要**拼音筛选**，请使用 `symbols` 类型。 |
...
| 函数名 | 说明 | 入参 | 返回值 |
| --- | --- | --- | --- |
| `$getRimeOptionState` | 获取 rime 引擎 option 的状态，如 `ascii_state` | rime 中 option 的值，如：`ascii_state` | 布尔值 |
| `$getRimePreedit` | 获取 rime 引擎中 preedit 字符串。 | 无 | 字符串 |
| `$getRimeCandidates` | 获取 rime 引擎中的候选字。 | 无 | 数组类型，其中每个元素都为一个 [Candidate 对象](#candidate-对象) |
| `$getSymbolicKeyboardLockState` | 获取符号键盘锁定状态 | 无 | 布尔值 |
| `$getReturnKeyType` | 当前回车键建议显示文字。 | 无 | 整数，表示系统推荐回车键显示文字。 |
...
- `rect` 表示图片的位置及大小信息，具体查阅 `rect` 属性。
- `insets` 表示**图片中不可被拉伸**的区域，它使用 top，bottom，left，right 四个方向描述，分别表示图片在这四个方向中，有多少区域是不可以被拉伸。具体查阅 insets 属性。如示例中的 `IMG1`，它的顶部（top），有 50 个像素是不可以被拉伸的。
...
| symbol | 字符串 | 直接上屏的字符，不会与 RIME 交互 | `{ symbol: w }`
`{ symbol: W }`
`{ symbol: abc }`  |
| shortcutCommand | 枚举 | 说明，键盘快捷指令，具体[查阅快捷指令](#快捷指令-shortcutcommand)。 | `{ shortcutCommand: "#重输" }` |
| sendKeys | 字符串 | 发送给 RIME 引擎的多个字符，具体[查阅 SendKey](#sendkeys)。 | `{ sendKeys: "Control+l" }` |
| openURL | 字符串 | 用于打开某个链接 | `{ openURL: "weixin://" }` |
| keyboardType | 枚举 | 用于切换键盘，具体[查阅键盘类型](#键盘类型)。 | `{ keyboardType: alphabetic }` |
...
| 值 | 说明 |
| --- | --- |
| `#简繁切换` 或 `#繁简切换` | 用于切换 RIME 引擎中的简体繁体，
注意：需要为系统配置当前输入方案简繁切换的 option 的值。
可查阅 hamster.yaml 中的 rime 下的 switchSimplifiedOrTraditionalOption 选项 |
...
| `#上个输入方案` | 切换当前输入方案为上个输入方案，若上个输入方案为空，则切换为排序为第二位的输入方案。
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| insets | 对象 | 调整预编辑区文本位置，具体查阅 insets 属性 |
...
| 属性 | 类型 | 说明 |
| --- | --- | --- |
| normalImage | 对象 | 具体查阅 normalImage 属性 |
...
| image | 字符串 | 图片描述文件中的键，具体请查阅[图片描述文件](#图片描述文件)。 |
...
> 关于图片裁剪和显示的逻辑，请[查阅图片的显示与裁剪](#图片裁剪与显示)。
...
| ranges | 数组类型，非必须 | 字体 Unicode 生效范围，可以为一个字体设置多个范围，具体查阅 ranges 参数。 |
...
一个比例值，用于设置中心点位置，从而调整显示位置。已 Cell 的左上角为原点
